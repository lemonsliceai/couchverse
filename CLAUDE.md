...
- **Persona count is server-driven.** Set `PERSONAS=a,b,c,d…` in `server/.env`. The API mints N rooms, the agent worker spins up N `AgentSession`s, and the extension renders an N-tile responsive grid. Adding a persona is: (1) drop a `persona_configs/{name}.py` with a `display=DisplayConfig(...)` block, (2) add the name to `PERSONAS` (first entry is primary), (3) restart server + agent. No extension rebuild required. The persona manifest (`GET /api/personas`) is versioned via `schema_version`; the extension validates this and refuses unknown versions.
