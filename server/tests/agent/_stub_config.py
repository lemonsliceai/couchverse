"""Shared in-memory ``PersonaConfig`` stubs for unit tests.

Tests should not load real preset files from ``persona_configs/`` — that
couples the test suite to whichever characters happen to ship today.
``make_stub_config(name)`` builds a minimal, fully-valid ``PersonaConfig``
...
def make_stub_config(name: str, *, label: str | None = None) -> PersonaConfig:
    """Build a minimal ``PersonaConfig`` for tests.
