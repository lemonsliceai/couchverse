...
from podcast_commentary.agent.verbalized_sampling import (
    JUDGE_SYSTEM,
    chunk_text,
    extract_transcript_block,
    parse_candidates,
    parse_judge_winner,
    prompt_uses_sampling,
    select_candidate,
)
...
class PersonaAgent(Agent):
    ...
    async def llm_node(
        ...
        if self._config.sampling.num_candidates <= 1 or not prompt_uses_sampling(chat_ctx):
        ...
        if self._config.sampling.selection == "judge":
            ...
            winner = select_candidate(raw, self._config.sampling.selection)
        yield winner
    ...
    async def _judge_select(self, raw: str, chat_ctx: llm.ChatContext) -> str:
        ...
        transcript = extract_transcript_block(chat_ctx)
        idx = -1
        try:
    ...
    async def _judge_pick(self, candidates: list[tuple[float, str]], *, transcript: str) -> int:
        ...
        chat_ctx.add_message(role="system", content=JUDGE_SYSTEM)
        ...
        return parse_judge_winner("".join(buf), len(candidates))
