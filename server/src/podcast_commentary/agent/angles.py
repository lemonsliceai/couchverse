...
prompt (see ``persona_configs/<persona>.py`` SYSTEM_PROMPT). The orchestrator
...
COMMENTARY_ANGLES: list[str] = list(CONFIG.character.comedic_angles)


def pick_angle(recent_angles: list[str], *, config: PersonaConfig | None = None) -> str:
    """Pick a commentary lens that wasn't used in the last few comments.

    The number of recent angles to avoid is configured by
    ``character.angle_lookback``. If that ever makes the pool empty (more
    angles excluded than available), fall back to the full bank.
    """
    cfg = config or CONFIG
    lookback = cfg.character.angle_lookback
    bank = list(cfg.character.comedic_angles)
